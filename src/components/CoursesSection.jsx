import React from 'react';
import { Award } from 'lucide-react';
import CoursesCard from './CoursesCard';
import { coursesData } from '../data/coursesData';

const CoursesSection = ({ visibleSections }) => {
	return (
		<section
			id="courses"
			className={`py-20 px-4 transition-all duration-1000 ${visibleSections.has('courses')
				? 'opacity-100 translate-y-0'
				: 'opacity-0 translate-y-10'
			}`}
		>
			<div className="max-w-6xl mx-auto">
				<h2 className="text-5xl font-bold mb-16 text-center bg-gradient-to-r from-purple-400 via-pink-400 to-blue-400 bg-clip-text text-transparent flex items-center justify-center gap-3">
					<Award className="text-purple-400" size={48} /> Курси
				</h2>

				<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
					{coursesData.map((course, index) => (
						<CoursesCard key={index} course={course} />
					))}
				</div>
			</div>
		</section>
	);
};

export default CoursesSection;